...
const MusicPlayer = ({ audioSrc }) => {
  ...
  }, [volume, audioSrc]);  return (
    <div className="fixed bottom-32 right-4 z-[100]">
      <audio ref={audioRef} src={audioSrc} preload="auto" loop />
        {audioError && (
        <div className="absolute bottom-16 right-1 bg-red-100 p-2 rounded-md text-xs text-red-600 w-32 hidden">
          {audioError}
        </div>
      )}
        {showVolume && (
        <div className="absolute bottom-14 right-1 bg-white/95 p-3 rounded-lg shadow-lg mb-2 backdrop-blur-sm border border-primary/20 w-28">
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className="w-11 h-11 bg-white/95 rounded-full shadow-lg flex items-center justify-center hover:bg-white transition-colors duration-300 border border-primary/20"
