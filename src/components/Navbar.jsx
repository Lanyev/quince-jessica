...
const Navbar = () => {  
  ...
  const NavButton = ({ onClick, icon, label, position }) => (
    <div className={`w-14 h-full flex items-center ${position === 'left' ? 'justify-start' : 'justify-end'}`}>
      <button
        onClick={onClick}
        className="w-12 h-12 flex items-center justify-center hover:bg-primary-light rounded-lg transition-all duration-200 active:scale-95"
        aria-label={label}
      >
        <div className="w-10 h-10 flex items-center justify-center rounded-full bg-primary-light bg-opacity-30 shadow-inner">
          {icon}
        </div>
      </button>
    </div>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 w-full mx-auto max-w-[calc(100%-30px)] bg-primary text-white py-3 px-4 rounded-xl" style={{ maxWidth: 'calc(100% - 30px)', marginLeft: 'auto', marginRight: 'auto', marginBottom: '15px', zIndex: 5 }}>
      <div className="flex justify-between items-center w-full h-[95px]">
            ...
            <AnimatePresence initial={false} mode="wait" custom={direction}>
              ...
              >
                ...
                {visibleItems.map((item, index) => (
                  <div key={`nav-${currentPage}-${index}`} className="flex-1 flex justify-center items-center">
                    <Link
                      to={item.to}
                      className="flex flex-col items-center justify-center p-1.5 hover:bg-primary-light rounded-lg transition-all duration-200 active:scale-95"
                    >
                      <div className="w-12 h-12 flex items-center justify-center rounded-full bg-primary-light bg-opacity-30 shadow-inner mb-1.5">
                        {item.icon}
                      </div>
                      <span className="text-sm font-nav text-center">{item.label}</span>
                    </Link>
                  </div>
                ))}
              </motion.div>
      ...
      <div className="flex justify-center mt-1">
        {Array.from({ length: totalPages }).map((_, i) => (
          <button 
            ...
            className={`w-2 h-2 mx-1 rounded-full ${currentPage === i ? 'bg-white' : 'bg-white/30'}`}
